...
function init () {
  ...
  var numberCount = parseFloat(sumMoney.innerHTML); 
  ...
  rightOuter.addEventListener('wheel',scrollFn);
  rightOuter.onmouseenter = function() {
    rightOuter.addEventListener('scroll',scrollFn);
  };

  function scrollFn() {
    for(var i=0;i<document.querySelectorAll('li[typeindex]').length;i++){
      if(rightOuter.scrollTop >= (arrOffsetTop[i] - liHeight)) {
  ...
  this.PlusMinus = function (obj) {
    ...
    function plusNumber(obj) {
      ...
      n = numberCount + parseFloat(price);
      numberCount = parseFloat(n.toFixed(2));
